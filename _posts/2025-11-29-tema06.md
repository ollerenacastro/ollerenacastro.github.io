---
title: Clase 06 - Buffer Overflows
...
**LINKS DE LA GRABACIÓN DE LA CLASE**: Clase 06 *(Pendiente)*

> **IMPORTANTE - Marco Ético y Legal**
>
> Las técnicas presentadas en este material deben utilizarse **ÚNICAMENTE** en entornos autorizados:
> - **Laboratorios controlados**: Máquinas virtuales propias o plataformas como Hack The Box, TryHackMe.
> - **Autorización escrita**: Siempre obtener permiso explícito antes de realizar pruebas de penetración.
> - **Legislación Perú**: Ley 30096 - Delitos Informáticos. Solo practicar en sistemas propios o con autorización expresa.
>
> El uso indebido de estas técnicas puede resultar en consecuencias legales graves.
...
> **Protecciones Modernas (2025)**
>
> Los sistemas operativos actuales implementan múltiples capas de defensa:
> - **CFG (Control Flow Guard)**: Verifica que las llamadas indirectas apunten a direcciones válidas.
> - **CET (Control-flow Enforcement Technology)**: Hardware shadow stack en procesadores Intel/AMD modernos.
> - **SMAP/SMEP**: Protecciones de hardware que previenen ejecución de código en memoria de usuario desde el kernel.
>
> Estas protecciones hacen que la explotación moderna sea significativamente más compleja que los ejemplos didácticos de este material.
...
> **NOTA: Registros 32-bit vs 64-bit**
>
> | 32-bit (x86) | 64-bit (x86_64) | Función |
> |--------------|-----------------|---------|
> | ESP | RSP | Stack Pointer |
> | EBP | RBP | Base Pointer |
> | EIP | RIP | Instruction Pointer |
>
> En este material usamos la nomenclatura de 32-bit (ESP, EBP, EIP) para el ejercicio con Vulnserver.
> En los ejemplos con GDB en Kali (sistema de 64-bit), verás los registros RSP, RBP, RIP.
...
### Visualización del Stack Frame

Antes de continuar con GDB, es crucial entender cómo se organiza el stack frame de `main()`. El siguiente diagrama muestra la estructura de memoria:


```
┌─────────────────────────────────────────────────────────────────┐
│              STACK FRAME DE main() - Sistema 64-bit             │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Direcciones ALTAS (hacia 0x7FFFFFFFFFFF)                       │
│  ┌─────────────────────────────────────┐                        │
│  │  Return Address (8 bytes)           │ ← RBP + 0x8            │
│  │  (dirección de retorno a libc)      │   ★ OBJETIVO ATAQUE ★  │
│  ├─────────────────────────────────────┤                        │
│  │  Saved RBP (8 bytes)                │ ← RBP apunta aquí      │
│  │  (base pointer del caller)          │   (0x7fffffffdca0)     │
│  ├─────────────────────────────────────┤                        │
│  │  int a = 2      (4 bytes)           │ ← RBP - 0x4            │
│  ├─────────────────────────────────────┤                        │
│  │  float b = 2.5  (4 bytes)           │ ← RBP - 0x8            │
│  ├─────────────────────────────────────┤                        │
│  │  (padding)      (4 bytes)           │ ← RBP - 0xC            │
│  ├─────────────────────────────────────┤                        │
│  │  int *ptr       (8 bytes)           │ ← RBP - 0x10 / RSP     │
│  │  (puntero al heap)                  │   (0x7fffffffdc90)     │
│  └─────────────────────────────────────┘                        │
│  Direcciones BAJAS (hacia 0x000000000000)                       │
│                                                                 │
├─────────────────────────────────────────────────────────────────┤
│  CÁLCULO CRÍTICO PARA BUFFER OVERFLOW:                          │
│                                                                 │
│  Distancia desde RSP hasta Return Address:                      │
│  = 16 bytes (variables locales) + 8 bytes (saved RBP)           │
│  = 24 bytes                                                     │
│                                                                 │
│  Si un buffer empezara en RSP y escribiéramos 25+ bytes,        │
│  sobrescribiríamos el Return Address y controlaríamos RIP.      │
└─────────────────────────────────────────────────────────────────┘
```

> **¿Por qué es importante?** En un ataque de buffer overflow, el atacante escribe datos más allá del espacio asignado. Como el stack crece hacia direcciones BAJAS pero los datos se escriben hacia direcciones ALTAS, un desbordamiento puede alcanzar y sobrescribir el **Return Address**, permitiendo al atacante redirigir la ejecución del programa.
...
- En este punto, se puede seguir observando los registros después de cada instrucción usando `info registers` para ver cómo cambian RSP, RBP, y RIP (en sistemas de 64-bit). Por ejemplo:

    - **RSP** (Stack Pointer): Disminuirá a medida que se almacenen variables locales en el stack.
    - **RBP** (Base Pointer): Se mantendrá constante dentro de una función, apuntando al inicio del stack frame.
    - **RIP** (Instruction Pointer): Cambiará en cada instrucción ejecutada, apuntando a la siguiente instrucción que se ejecutará.

> **NOTA:** En sistemas de 32-bit, estos registros se llaman ESP, EBP, y EIP respectivamente. En este ejemplo usamos un sistema de 64-bit (Kali Linux), por lo que verás RSP, RBP, y RIP.
...
- Este comando (`x/20x $rsp`) muestra 20 palabras (words) de 4 bytes cada una (80 bytes total) a partir de la dirección a la que apunta RSP. A medida que avancemos en la ejecución, veremos cómo las variables locales y las direcciones de retorno se almacenan en el stack.

- Ejemplo en el Código:
    - En el código proporcionado, cuando se llama a `malloc`, se reserva memoria en el heap, pero las variables locales como `a` y `b` (enteros y flotantes) se almacenan en el stack. Al ejecutar el programa, podrás observar cómo RSP y RBP gestionan el stack frame de `main()` y cómo RIP avanza a medida que se ejecutan las instrucciones.

- Resumen (Registros de 64-bit)
    - **RSP** (Stack Pointer): Apunta a la parte superior del stack y cambia constantemente conforme se añaden o eliminan elementos del stack.
    - **RBP** (Base Pointer): Marca el inicio de un stack frame y se usa como referencia para acceder a las variables locales de una función.
    - **RIP** (Instruction Pointer): Contiene la dirección de la siguiente instrucción que se ejecutará y cambia después de cada instrucción ejecutada.

### Conexión con Buffer Overflow: El "Momento Aha"

El ejemplo anterior muestra un programa **seguro** (sin buffers vulnerables). Pero, ¿qué pasaría si el código tuviera un buffer que pudiera desbordarse?

**Ejemplo de código vulnerable:**
```c
void vulnerable(char *input) {
    char buffer[64];        // Buffer de 64 bytes en el stack
    strcpy(buffer, input);  // ¡SIN verificación de límites!
}
```

En este caso, el stack frame se vería así:

```
┌────────────────────────────────────────────┐
│  Return Address    (8 bytes)               │ ← OBJETIVO
├────────────────────────────────────────────┤
│  Saved RBP         (8 bytes)               │
├────────────────────────────────────────────┤
│  buffer[63]                                │
...
│  ...                                       │ 64 bytes
│  buffer[1]                                 │
│  buffer[0]         ← Escritura empieza aquí│
└────────────────────────────────────────────┘

ATAQUE: Si escribimos 64 + 8 + 8 = 80 bytes, sobrescribimos el Return Address
```

**El insight clave:**
> Si `strcpy` copia más de 64 bytes en `buffer`, los bytes adicionales sobrescriben primero el Saved RBP, y luego el **Return Address**. Cuando la función ejecuta `ret`, en lugar de retornar al caller original, salta a la dirección que el atacante escribió.

**Conexión con Vulnserver (secciones siguientes):**
- En Vulnserver, el comando TRUN tiene un buffer interno
- Mediante fuzzing, descubrimos que el offset es **2003 bytes**
- Esto significa: 2003 bytes de buffer + datos del stack = distancia hasta EIP
- Una vez que controlamos EIP, podemos redirigir la ejecución a nuestro shellcode

<hr style="border: none; height: 10px; background-color: #003b00;" />

## <font color="#87CEEB">4. Pasos para un Desbordamiento de Búfer.</font>
...
## <font color="#87CEEB">5. Spiking.</font>

<hr style="border: none; height: 10px; background-color: #003b00;" />

> **NOTA SOBRE EL ENTORNO DE LABORATORIO**
>
> Este ejercicio utiliza **Vulnserver**, un binario deliberadamente vulnerable diseñado para aprendizaje.
> Las protecciones modernas (DEP, ASLR, Stack Canaries) están **deshabilitadas** en este entorno.
> En sistemas reales con protecciones activas, se requieren técnicas avanzadas como **ROP (Return-Oriented Programming)** para lograr la explotación.
...
## <font color="#87CEEB">6. Fuzzing.</font>
...
## <font color="#87CEEB">7. Encontrando el Offset.</font>
...
## <font color="#87CEEB">8. Sobrescribiendo el EIP.</font>
...
## <font color="#87CEEB">9. Identificando Caracteres Malos.</font>
...
## <font color="#87CEEB">10. Buscando el módulo vulnerable.</font>
...
- Y en el Immunity Debugger, obtendremos el overwriting del EIP:
...
## <font color="#87CEEB">11. Generación de Shellcode.</font>
