---
title: Clase 05 - Caldera Framework - Emulación de Adversarios con MITRE ATT&CK
...
Para este laboratorio, se seleccionarán las siguientes habilidades:
...
name: Lab3-DataThief
description: Adversario de laboratorio que simula descubrimiento, acceso a credenciales y recolección de datos
atomic_ordering:
  # Fase 1: Discovery
...
**Uso táctico**: Mapear hosts activos en la red local para potencial movimiento lateral.
...
**Uso táctico**: Determinar contexto de privilegios y plataforma para adaptar técnicas posteriores.
...
**Uso táctico**: Identificar segmentos de red accesibles.
...
**Uso táctico**: Claves SSH descubiertas pueden usarse para acceso a otros sistemas.
...
**Uso táctico**: Credenciales en histórico bash pueden reutilizarse para acceso lateral.
...
**Uso táctico**: Identificar datos de alto valor para exfiltración.
...
**Uso táctico**: Directorio para agrupar archivos antes de exfiltración (técnica real de APTs).
...
```
[T1018 ARP Scan] → Descubre IPs remotas
       ↓
[T1082 System Info] → Descubre usuario/OS
       ↓
[T1016 Network Config] → Descubre interfaces
       ↓
[T1552.004 SSH Keys] → Encuentra claves privadas
       ↓
[T1552.003 Bash History] → Extrae credenciales
       ↓
[T1005 Sensitive Files] → Identifica archivos objetivo
       ↓
[T1074.001 Staging Dir] → Prepara exfiltración
```
...
| Command and Control | T1071 (implícito por C2) | 1/16 (6%) |

**Observación**: Este adversario cubre 4 de 14 tácticas ATT&CK, con enfoque en fases tempranas del kill chain.
...
3. **Creación de directorios sospechosos**:
   ```
...
Un EDR configurado adecuadamente debería alertar sobre:
- Ejecución secuencial rápida de comandos de descubrimiento
- Acceso a `.bash_history` por proceso no interactivo
- `find` buscando patrones de claves privadas
- Creación de directorios ocultos en `/tmp`

### Ejercicio de extensión

#### Agregar fase de exfiltración
...
**Habilidad de exfiltración** (ejemplo):
